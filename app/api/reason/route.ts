...
export async function POST(request: NextRequest) {
  try {
    ...
    // Dynamic system prompt based on establishment type
    const establishmentType = body.establishmentType || 'cafe';
    const typeLabels = {
      cafe: { singular: 'café', plural: 'cafés', name: 'coffee shops' },
      restaurant: { singular: 'restaurant', plural: 'restaurants', name: 'restaurants' },
      museum: { singular: 'museum', plural: 'museums', name: 'museums' },
      bar: { singular: 'bar', plural: 'bars', name: 'bars' },
    };
    const labels = typeLabels[establishmentType as keyof typeof typeLabels] || typeLabels.cafe;

    const systemPrompt = `You are Loca, an AI that matches ${labels.name} based on their characteristics, vibe, community reputation, and visual aesthetics.

Your job is to create a compelling, personality-rich explanation of why a ${labels.singular} matches another.

IMPORTANT FORMAT:
- First sentence: Start with a SHORT, punchy descriptor (3-5 words max) that captures the ${labels.singular}'s most dominant characteristic
- Following sentences: Explain the match using multiple data sources: Google ratings, Reddit community sentiment, visual style, reviewer personality types, location characteristics
